...
async function migrateData() {
  ...
  try {
    ...
    if (parseInt(existingMovies.rows[0].count) > 0) {
      console.log('Movies already exist in database. Skipping movie migration.');
    } else {
      console.log('Migrating movies...');
      
      for (const movie of moviesData) {
        const movieResult = await client.query(
          `INSERT INTO movies (title, description, year, rating, poster_url, trailer_url, part)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           RETURNING id`,
          [movie.title, movie.description, movie.year, movie.rating, movie.posterUrl, movie.trailerUrl, movie.part || null]
        );
        ...
        for (const castMember of movie.cast) {
          let castResult = await client.query(
            'SELECT id FROM cast_members WHERE name = $1',
            [castMember.name]
          );
          ...
          if (castResult.rows.length === 0) {
            const insertResult = await client.query(
              'INSERT INTO cast_members (name, profile_url) VALUES ($1, $2) RETURNING id',
              [castMember.name, castMember.avatarUrl]
            );
            castId = insertResult.rows[0].id;
          } else {
            castId = castResult.rows[0].id;
          }
          
          await client.query(
            'INSERT INTO movie_cast (movie_id, cast_id, role) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING',
            [movieId, castId, 'Actor']
          );
        }
      ...
      console.log(`Successfully migrated ${moviesData.length} movies!`);
    }
    
    const existingComments = await client.query('SELECT COUNT(*) FROM comments');
    if (parseInt(existingComments.rows[0].count) > 0) {
      console.log('Comments already exist in database. Skipping comments migration.');
    } else {
      ...
      for (const comment of commentsData) {
        ...
        if (userResult.rows.length === 0) {
          const insertResult = await client.query(
            ...
            [comment.user, `${comment.user.toLowerCase().replace(/\s+/g, '.')}@example.com`, comment.avatar]
          );
        ...
        const movies = await client.query('SELECT id FROM movies ORDER BY RANDOM() LIMIT 1');
        const movieId = movies.rows[0].id;
        
        await client.query(
          'INSERT INTO comments (movie_id, user_id, text) VALUES ($1, $2, $3)',
          [movieId, userId, comment.text]
        );
        
        console.log(`  ✓ Migrated comment by: ${comment.user}`);
      }
      
      console.log(`Successfully migrated ${commentsData.length} comments!`);
    }
