...
interface FooterProps {
  ...
  onScrollToSection: (section: 'newReleases' | 'genres' | 'allMovies') => void;
  isAuthenticated: boolean;
}

const FooterLink: React.FC<{ onClick: () => void; children: React.ReactNode }> = ({ onClick, children }) => (
  <button onClick={onClick} className="text-gray-500 dark:text-text-muted hover:text-black dark:hover:text-text-primary transition-colors duration-300 text-left">
    {children}
  </button>
);
...
const FooterComponent: React.FC<FooterProps> = ({ onNavigate, onScrollToSection, isAuthenticated }) => {
  return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="space-y-4">
            ...
            <ul className="space-y-3">
              <li><FooterLink onClick={() => onScrollToSection('genres')}>Genres</FooterLink></li>
              <li><FooterLink onClick={() => onScrollToSection('newReleases')}>New Releases</FooterLink></li>
              <li><FooterLink onClick={() => onScrollToSection('allMovies')}>All Movies</FooterLink></li>
            </ul>
          </div>
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-500 dark:text-text-secondary tracking-wider uppercase">Support</h3>
            <ul className="space-y-3">
              <li><FooterLink onClick={() => onNavigate('help')}>Help Center</FooterLink></li>
              <li><FooterLink onClick={() => onNavigate('howToDownload')}>How to Download</FooterLink></li>
              <li><FooterLink onClick={() => onNavigate('contact')}>Contact Us</FooterLink></li>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3">
              {isAuthenticated ? (
                <>
                  <li><FooterLink onClick={() => onNavigate('profile')}>My Profile</FooterLink></li>
                  <li><FooterLink onClick={() => onNavigate('downloader')}>My Downloads</FooterLink></li>
                  <li><FooterLink onClick={() => onNavigate('profile')}>Settings</FooterLink></li>
                </>
              ) : (
                <li><FooterLink onClick={() => onNavigate('login')}>Login / Sign Up</FooterLink></li>
              )}
            </ul>
          </div>
          <div className="space-y-4">
            ...
            <ul className="space-y-3">
              <li><FooterLink onClick={() => onNavigate('terms')}>Terms of Service</FooterLink></li>
              <li><FooterLink onClick={() => onNavigate('privacy')}>Privacy Policy</FooterLink></li>
              <li><FooterLink onClick={() => onNavigate('dmca')}>DMCA Notice</FooterLink></li>
              <li><FooterLink onClick={() => onNavigate('cookiePolicy')}>Cookie Policy</FooterLink></li>
            </ul>
          </div>
        </div>
        <div className="mt-10 pt-8 border-t border-gray-200 dark:border-background-secondary flex flex-col sm:flex-row justify-between items-center">
