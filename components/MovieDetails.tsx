...
const MovieDetailsComponent: React.FC<MovieDetailsProps> = ({ movie, userProfile, onBack, onDownloadClick, onSelectMovie }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const fetchedComments = await api.getMovieComments(movie.id);
  ...
  const [movieSeries, setMovieSeries] = useState<Movie[]>([]);
  ...
  useEffect(() => {
    const fetchMovieSeries = async () => {
      ...
      try {
        const response = await api.getMovies({ search: movie.title });
        if (response.movies) {
          const series = response.movies
            .filter(m => m.part && m.title === movie.title)
            .sort((a, b) => (a.part || 0) - (b.part || 0));
          setMovieSeries(series);
        }
      } catch (error) {
        console.error('Failed to fetch movie series:', error);
        setMovieSeries([]);
      }
    };
    
    fetchMovieSeries();
  }, [movie.id, movie.part, movie.title]);
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    if (newComment.trim() && userProfile.id) {
      try {
        const comment = await api.addComment(movie.id, userProfile.id, newComment.trim());
  ...
  return (
    <div className="animate-fade-in">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 pt-16 md:pt-20">
        {/* Back button */}
        <div className="mb-8 md:mb-10">
            <button onClick={onBack} className="flex items-center text-gray-800 dark:text-text-secondary hover:text-black dark:hover:text-white font-semibold transition-colors duration-300 group">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-12">
            ...
            <div className="md:w-1/3 flex-shrink-0">
                 <div className="relative w-full rounded-lg shadow-2xl dark:shadow-black/40 overflow-hidden bg-gray-200 dark:bg-background-tertiary">
                    {!isPosterLoaded && <div className="absolute inset-0 bg-background-tertiary animate-pulse" />}
                    <img
                        src={movie.posterUrl}
                        alt={`Poster for ${movie.title}`}
                        className={`w-full h-auto object-cover transition-opacity duration-300 ${isPosterLoaded ? 'opacity-100' : 'opacity-0'}`}
                        loading="lazy"
                        decoding="async"
            ...
            <div className="md:w-2/3">
                ...
                <div className="mt-6">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-text-primary mb-3">Synopsis</h2>
        ...
        <div className="mt-10 space-y-8">
                ...
                <Section title="Parts">
                    <div className="flex flex-wrap gap-3">
                        {movieSeries.map(partMovie => (
                            <button
                                key={partMovie.id}
                                onClick={() => onSelectMovie(partMovie)}
            ...
            {/* Cast Section */}
            <Section title="Cast">
                <div className="flex space-x-4 overflow-x-auto pb-2 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8" style={{ scrollbarWidth: 'none', '-ms-overflow-style': 'none' }}>
            ...
            <Section title="Comments">
                <div className="max-w-2xl">
                    <form onSubmit={handleCommentSubmit} className="mb-6">
                        ...
                        />
                        <button type="submit" className="mt-3 px-6 py-2 bg-gray-800 dark:bg-white text-white dark:text-black font-bold rounded-full hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed" disabled={!userProfile}>
                            Post Comment
                        </button>
                    </form>
